...
def read_imu_data_remote():
    """
    Connects to remote host, downloads the IMU_recent.csv file, and filters data (last TIME_WINDOW seconds).
...
def read_vlp_data_remote():
    """
    Connects to remote host, downloads the VLP_recent.csv file, and filters data (last TIME_WINDOW seconds).
...
def read_gt_data(GT_Plot_points):
    """
    Read the latest GT CSV file from GT_DIR and return a DataFrame consisting of the last 300 records.
